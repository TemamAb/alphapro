...
def test_strategy_route(mock_redis, mock_db, client):
    ...
    assert data['name'] == 'Dynamic Arbitrage'
    assert 'leverage' in data['parameters']
    assert 'riskTolerance' in data['parameters']
    assert 1 <= data['parameters']['leverage'] <= 5
    assert data['parameters']['riskTolerance'] in ['Low', 'Medium', 'High']
